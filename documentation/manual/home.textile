...
Siena module simply uses database connectors configured and provided by Play.
...
bc. #uses embedded H2 in memory provided by Play
db=mem

bc. #uses embedded H2 in file provided by Play 
db=mem    
...
h3. <a>Configure SQL database DDL creation/update</a>
...
p(note). **Note** JPA parameters are not used at all.    
...
This documentation is no more up to date as Siena has been deeply enhanced for v1.0.0 and we are going to write much more docs very soon. Meanwhile, here are a few information to begin.

h3. A classical Siena model with auto-generated Long ID

p(note). **Important remarks** 
...
h3. A classical Siena model with manual String ID
...
h3. A classical Siena model with auto-generated Long ID with a non-"id" name 
...
h3. A classical Siena model with UUID String ID
...
h3. An enhanced Siena model for the lazy people
